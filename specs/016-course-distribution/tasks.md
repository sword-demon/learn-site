...
- [X] T001 [P] 在 `apps/api/database/migrations/` 下新建 `20260906000001_distribution.php` (仅占位类, 待 Foundational 阶段填充建表)
- [X] T002 [P] 在 `apps/api/database/seeds/PermissionSeeder.php` 的 PERMISSIONS 数组末尾追加 3 行 distribution 权限点 (占位, 待 Foundational 阶段确认值)
- [X] T003 [P] 在 `packages/contracts/src/` 下新建空 `distribution.ts` 与 `index.ts` 暂不动 (待 US1 起填 schema)
...
## Phase 3: User Story 1 - 学员生成只属于自己的专属分享链接 (Priority: P1) 🎯 MVP
...
**Goal**: 订单 succeeded 钩子触发 CommissionService::settleForOrder 写入 pending; 退款窗口结束 markSettledForOrder 将 pending → settled; 沿 referee 链路取 ≤ 3 名推荐人; 单笔去重 ≤ 3; 退款时 voidForOrder 撤销; 推荐人账户不可用时不升级不转赠. 每笔成功订单都结算, 不以该课首单为条件.
...
- 金额一律 int cents, 库内时间 DATETIME (`Asia/Shanghai`), API/Zod 时间 ISO-8601, 手机号一律 `^1[3-9]\d\*{4}\d{4}$` 脱敏
- 管理端 seeder 权限点 3 个 (`config` / `reconcile` / `audit`); 撤销挂在 reconcile
- 首版结算: succeeded → pending; 退款窗口结束无退款 → settled; 退款 → voided
...
- 总任务数: 95
- 各故事任务数: US1=11, US2=12, US3=13, US4=12, US5=12, US6=8
