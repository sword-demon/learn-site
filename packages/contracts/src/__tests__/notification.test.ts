...
describe("notification contracts", () => {
  it("accepts progress catalog changes", () => {
    expect(LearnerNotificationKind.parse("progress_catalog_changed")).toBe(
  ...
  it("accepts the course_published kind pointing at a course", () => {
    expect(LearnerNotificationKind.safeParse("course_published").success).toBe(
      true,
    );
    const parsed = LearnerNotificationDTO.parse({
      id: 2,
