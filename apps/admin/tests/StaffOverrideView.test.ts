...
describe('StaffOverrideView', () => {
  beforeEach(() => {
    ...
    orgApi.getStaff.mockResolvedValue(staff([
      { effect: 'deny', code: 'order.view', permission_id: 1 },
    ]))
  ...
  it('loads existing overrides and sends the complete replacement set', async () => {
    const wrapper = mount(StaffOverrideView)
    await flushPromises()
  ...
  it('disables permissions the operator does not hold before save', async () => {
    ...
    expect(unavailable.every((button) => (button.element as HTMLButtonElement).disabled)).toBe(true)
  ...
  it('keeps a super administrator read-only and shows API errors', async () => {
    ...
    expect(effectButtons.every((button) => (button.element as HTMLButtonElement).disabled)).toBe(true)
