...
import {
  ...
  CourseDeletionResult as CourseDeletionResultSchema,
  ChapterDTO as ChapterSchema,
  LessonDTO as LessonSchema,
  PaginatedCategories as PaginatedCategoriesSchema,
...
const CategoryIndexEnvelope = ApiOk(
  z.object({
...
const CategoryFlatEnvelope = ApiOk(PaginatedCategoriesSchema);
...
export async function listCategoriesFlat(
  ...
  const response = await http.get<unknown>('/categories/flat', {
    params,
  });
  return CategoryFlatEnvelope.parse(response.data).data;
}
...
export async function setCategoryStatus(
  ...
  const response = await http.patch<unknown>(`/categories/${id}/status`, input);
  return CategoryEnvelope.parse(response.data).data;
}
...
export async function updateCourse(id: number, input: UpdateCourseInput): Promise<CourseDTO> {
...
export async function deleteCourse(id: number): Promise<CourseDeletionResult> {
  const response = await http.delete<unknown>(`/courses/${id}`);
  return CourseDeleteEnvelope.parse(response.data).data;
}

// ─── Chapters ─────────────────────────────────────────────────────────

export async function listChapters(courseId: number): Promise<{ items: ChapterDTO[] }> {
  const response = await http.get<unknown>(`/courses/${courseId}/chapters`);
...
export async function createChapter(
  ...
  const response = await http.post<unknown>(`/courses/${courseId}/chapters`, input);
...
export async function updateChapter(
  ...
  const response = await http.patch<unknown>(`/courses/${courseId}/chapters/${chapterId}`, input);
  return ChapterEnvelope.parse(response.data).data;
}
...
export async function listLessons(
  ...
  const response = await http.get<unknown>(`/courses/${courseId}/lessons`, {
    params: { chapter_id: chapterId },
  });
  return LessonIndexEnvelope.parse(response.data).data;
}

export async function createLesson(courseId: number, input: CreateLessonInput): Promise<LessonDTO> {
  const response = await http.post<unknown>(`/courses/${courseId}/lessons`, input);
...
export async function updateLesson(
  ...
  const response = await http.patch<unknown>(`/courses/${courseId}/lessons/${lessonId}`, input);
  return LessonEnvelope.parse(response.data).data;
}
