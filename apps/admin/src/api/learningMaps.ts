...
const DeleteStageEnvelope = ApiOk(
  z.object({
    id: z.number().int().positive(),
    stage_id: z.number().int().positive(),
  }),
);
const CourseStepEnvelope = ApiOk(
  z.object({
    id: z.number().int().positive(),
    stage_id: z.number().int().positive(),
...
const RemoveCourseEnvelope = ApiOk(
  z.object({
    stage_id: z.number().int().positive(),
    course_id: z.number().int().positive(),
  }),
...
export async function addStage(mapId: number, input: CreateStageInput): Promise<MapStageDTO> {
  const response = await http.post<unknown>(`/learning-maps/${mapId}/stages`, input);
...
export async function updateStage(
  ...
  const response = await http.patch<unknown>(`/learning-maps/${mapId}/stages/${stageId}`, input);
  return MapStageEnvelope.parse(response.data).data;
}
