  ...
  <section class="page">
    ...
    >
      ...
      >
        <template #default="{ row }">
          <el-tag
            :type="row.account_status === 'active' ? 'success' : 'info'"
            effect="light"
          >
...
const canManageOverrides = computed(() => permissionCodes().includes('*') || permissionCodes().includes('org.grant'))
...
async function onDelete(row: StaffDTO): Promise<void> {
  try {
    await ElMessageBox.confirm(
      `确定删除员工「${row.display_name || row.login}」吗？`,
      '确认',
      { type: 'warning' },
    )
  } catch {
