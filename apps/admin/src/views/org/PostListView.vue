<template>
  <section class="page">
    <header class="bar">
      <h2>岗位管理</h2>
      <div class="actions">
        <el-button type="primary" :disabled="departments.length === 0" @click="openCreate">
          新增岗位
        </el-button>
    ...
    <el-table v-loading="loading" :data="rows" stripe row-key="id" class="table">
      <el-table-column prop="name" label="岗位名称" min-width="220" />
      <el-table-column prop="department_name" label="所属部门" min-width="200" />
      <el-table-column label="默认角色" min-width="220">
        <template #default="{ row }">
          <div class="role-tags">
            <el-tag v-for="name in roleNames(row.role_ids)" :key="name" effect="plain">
              {{ name }}
            </el-tag>
...
async function loadSupportData(): Promise<void> {
  try {
    const [departmentResult, roleResult] = await Promise.all([listDepartments(), listRoles()]);
