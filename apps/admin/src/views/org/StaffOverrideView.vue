...
<template>
  <section class="page override-view">
    <header class="head">
      <button type="button" class="btn back" @click="back">返回员工列表</button>
      <h1 class="display">
        权限覆盖 ·
    ...
    <template v-else-if="detail">
        ...
        <table class="grid">
          <thead>
            <tr>
              ...
              <th>模块</th>
              <th>说明</th>
              <th class="actions">覆盖</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="p in allPermissions" :key="p.id">
              ...
              <td class="actions">
                <button
                  type="button"
                  class="btn"
                  :class="{ active: buffer[p.code] === 'grant' }"
                  :disabled="detail.staff.is_super_admin || !hasPermission(p.code)"
                  :title="hasPermission(p.code) ? '授予或清除覆盖' : '当前账号未持有此权限'"
                  @click="setEffect(p.code, buffer[p.code] === 'grant' ? null : 'grant')"
                >
                  允许
                </button>
                <button
                  type="button"
                  class="btn deny"
                  :class="{ active: buffer[p.code] === 'deny' }"
                  :disabled="detail.staff.is_super_admin || !hasPermission(p.code)"
                  :title="hasPermission(p.code) ? '禁用或清除覆盖' : '当前账号未持有此权限'"
                  @click="setEffect(p.code, buffer[p.code] === 'deny' ? null : 'deny')"
                >
                  禁止
                </button>
              </td>
            ...
            <tr v-if="allPermissions.length === 0">
              <td colspan="4" class="empty">暂无可配置权限</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <footer class="footer">
        ...
        <button type="button" class="btn btn-primary" :disabled="!dirty || saving" @click="saveAll">
          保存覆盖
        </button>
      </footer>
    </template>
...
.display { margin: 0; font-size: 1.3rem; }
.hint { color: var(--color-text-muted, #5b6472); margin: 0; font-size: 0.85rem; }
.notice { color: var(--color-text-muted, #5b6472); margin: 0; }
.notice.error { color: #b42318; }
.notice.warn { color: #b45309; }
.grid-wrap { overflow-x: auto; }
.grid { width: 100%; min-width: 680px; border-collapse: collapse; font-size: 0.9rem; background: #fff; border: 1px solid var(--color-border, #d0d4dc); border-radius: 6px; overflow: hidden; }
.grid th, .grid td { border-bottom: 1px solid var(--color-border, #e6e8ee); padding: 8px 12px; text-align: left; }
.grid th { background: var(--color-bg-soft, #fafbfd); font-weight: 600; }
.actions { display: flex; gap: 6px; }
.actions .btn { padding: 4px 10px; }
.empty { color: var(--color-text-muted, #5b6472); text-align: center !important; }
...
.btn.active { background: var(--color-primary, #2563eb); color: #fff; border-color: transparent; }
.btn.deny.active { background: #b42318; color: #fff; border-color: transparent; }
.btn-primary { background: var(--color-primary, #2563eb); color: #fff; border-color: transparent; }
.btn.back { align-self: start; width: max-content; }
.footer { display: flex; justify-content: space-between; align-items: center; gap: 12px; flex-wrap: wrap; }
.muted { color: var(--color-text-muted, #5b6472); font-size: 0.85rem; }
</style>
