<template>
  <section class="page">
    ...
    <el-tabs v-model="activeTab" class="tabs">
      <el-tab-pane label="基本信息" name="basic">
        <el-form :model="form" label-position="top" class="form-grid">
          <el-form-item label="标题" required>
            <el-input v-model="form.title" clearable maxlength="128" placeholder="1–128 字" />
          </el-form-item>
          <el-form-item label="课程封面">
            <CourseCoverUpload v-model="form.cover_url" :upload="uploadCourseCover" />
          </el-form-item>
          ...
          <p v-if="departmentOptions.length === 0" class="field-hint span-2">
            暂无启用部门，请先在组织管理 → 部门管理创建。
          </p>
          <el-form-item label="讲师" required>
            <el-input v-model="form.teacher_name" clearable maxlength="64" placeholder="1–64 字" />
          </el-form-item>
          ...
          <el-form-item label="简介" class="span-2">
            <el-input v-model="form.summary" clearable maxlength="255" placeholder="≤255 字" />
          </el-form-item>
          <el-form-item label="富文本简介 (HTML)" class="span-2" required>
            <ContentEditor
              v-model="form.intro_rich_text"
              placeholder="请输入课程简介（支持格式化文本、图片、视频等）"
              :height="400"
            />
          </el-form-item>
          ...
          <el-form-item v-if="form.price_mode === 'paid'" label="列表价">
            <el-input-number v-model="form.list_price" :min="0" :max="99999.99" :precision="2" />
          </el-form-item>
          <el-form-item v-if="form.price_mode === 'paid'" label="销售价（可空）">
            <el-input-number v-model="form.sale_price" :min="0" :max="99999.99" :precision="2" />
          </el-form-item>
          <el-form-item
      ...
      <el-tab-pane label="章节与课节" name="structure">
        <div class="structure">
          <aside class="chapters">
            ...
            <ul>
              ...
              >
                ...
                <el-dropdown trigger="click" @command="(c: string) => onChapterCommand(c, ch)">
                  <el-button link size="small"> ··· </el-button>
                  <template #dropdown>
                    <el-dropdown-menu>
                      <el-dropdown-item command="edit"> 编辑 </el-dropdown-item>
                      <el-dropdown-item command="delete" :disabled="ch.lessons.length > 0">
                        删除
                      </el-dropdown-item>
                    </el-dropdown-menu>
                  </template>
          ...
          <section class="lessons">
            ...
            <el-table :data="selectedChapter?.lessons ?? []" row-key="id">
              ...
              <el-table-column label="操作" width="200">
                <template #default="{ row }">
                  <el-button link size="small" type="primary" @click="openLessonDialog(row)">
                    编辑
                  </el-button>
                  <el-button link size="small" type="danger" @click="onDeleteLesson(row)">
                    删除
                  </el-button>
                </template>
              </el-table-column>
      ...
      <el-tab-pane label="预览" name="preview">
        <div v-if="tree.id" class="preview">
          <h2>{{ tree.title }}</h2>
          <p class="muted">
            讲师：{{ tree.teacher_name }} · {{ formatPrice(tree) }} ·
            <el-tag size="small">
          ...
          <div v-if="tree.intro_rich_text" class="rich" v-html="tree.intro_rich_text" />
          <el-collapse>
        ...
        <el-empty v-else description="先保存草稿后可预览" />
      </el-tab-pane>
    </el-tabs>
    ...
    <el-dialog
      ...
      width="420px"
    >
      <el-form label-position="top" :model="chapterDialog">
        <el-form-item label="标题" required>
          <el-input v-model="chapterDialog.title" clearable maxlength="128" />
        </el-form-item>
        <el-form-item label="排序">
          <el-input-number v-model="chapterDialog.sort" :min="0" :max="999" />
        </el-form-item>
        <el-form-item v-if="chapterDialog.id !== null" label="状态">
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="chapterDialog.saving" @click="saveChapter">
          保存
        </el-button>
      </template>
    </el-dialog>
    ...
    >
      <el-form label-position="top" :model="lessonDialog">
        <el-form-item label="标题" required>
          <el-input v-model="lessonDialog.title" clearable maxlength="128" />
        </el-form-item>
        <el-form-item label="排序">
          <el-input-number v-model="lessonDialog.sort" :min="0" :max="999" />
        </el-form-item>
        <el-form-item label="内容类型" required>
          <el-radio-group v-model="lessonDialog.content_type">
            <el-radio-button value="markdown"> Markdown </el-radio-button>
            <el-radio-button value="pdf"> PDF 资源 </el-radio-button>
            <el-radio-button value="video"> 视频资源 </el-radio-button>
          </el-radio-group>
        </el-form-item>
        <el-form-item v-if="lessonDialog.content_type === 'markdown'" label="正文 (Markdown)">
          <el-input v-model="lessonDialog.body_markdown" clearable type="textarea" :rows="6" />
        </el-form-item>
        <el-form-item v-else label="上传资源">
          <div class="upload-row">
            ...
            <span v-else class="asset-id">asset_id: {{ lessonDialog.asset_id }}</span>
            <el-button v-if="lessonDialog.asset_id" link @click="lessonDialog.asset_id = null">
              重新选择
            </el-button>
          </div>
        </el-form-item>
        <el-form-item label="时长 (秒)">
          <el-input-number v-model="lessonDialog.duration_seconds" :min="0" :max="360000" />
        </el-form-item>
        <el-form-item label="试看">
          <el-switch v-model="lessonDialog.is_preview" />
        </el-form-item>
        <el-form-item v-if="lessonDialog.id !== null" label="状态">
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="lessonDialog.saving" @click="saveLesson">
          保存
        </el-button>
      </template>
    </el-dialog>
    ...
    if (lessonDialog.id === null) {
      ...
      const input: UpdateLessonInput = {
        ...
        body_markdown:
          lessonDialog.content_type === 'markdown' ? lessonDialog.body_markdown : undefined,
        asset_id:
          lessonDialog.content_type !== 'markdown' && lessonDialog.asset_id !== null
            ? lessonDialog.asset_id
            : undefined,
        is_preview: lessonDialog.is_preview,
        duration_seconds: lessonDialog.duration_seconds,
...
watch(
  () => [route.fullPath, form.title, isNew.value] as const,
  () => {
    if (isNew.value) return;
    const label = form.title.trim();
    if (!label) return;
    tabsStore.updateTitle(route.fullPath, `编辑：${label}`);
  },
);
