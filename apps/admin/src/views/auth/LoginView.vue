...
import { AdminLoginInput, ApiResponse, CaptchaChallenge } from '@learn-site/contracts'
...
async function onSubmit(): Promise<void> {
  ...
  try {
    const res = await http.post<{ ok: true; data: TokenPair & { must_change_password?: boolean } }>(
      '/auth/login',
      parsed.data,
    )
    if (!res.data.ok) {
    ...
    if (res.data.data.must_change_password === true) {
