...
/**
 * One order pays one referrer at most once: the app settles idempotently and
 * the UNIQUE (order_id, referrer_learner_id) index backstops a race.
 */
final class CommissionReceiverUniquenessTest extends TestCase
{
    ...
    public function testDoubleSettleProducesSingleRecordSet(): void
    {
        $ids = $this->chain(2);
        $this->enable();
        $orderId = $this->succeededOrder($ids[1], 100.00);

        $svc = new CommissionService();
        $svc->settleForOrder($orderId);
        $svc->settleForOrder($orderId); // replayed request — must be a no-op

        self::assertSame(
            1,
            (int) Db::name('commission_records')->where('order_id', $orderId)->count(),
        );
    }

    public function testUniqueIndexRejectsDuplicateReceiverRow(): void
    {
        $ids = $this->chain(2);
        $this->enable();
        $orderId = $this->succeededOrder($ids[1], 100.00);
        ...
        $duplicate['id'] = null;
        try {
            Db::name('commission_records')->insert($duplicate);
            self::fail('UNIQUE (order_id, referrer_learner_id) must reject a duplicate receiver row');
        } catch (\Throwable $e) {
            $message = strtolower($e->getMessage());
            self::assertTrue(
                str_contains($message, 'duplicate') || str_contains($message, 'unique'),
                'expected a UNIQUE violation, got: ' . $e->getMessage(),
            );
        }
        self::assertSame(
            1,
    ...
    private function chain(int $n): array
    {
        ...
        for ($i = 0; $i < $n; $i++) {
            $id = (int) Db::name('accounts')->insertGetId([
                'kind' => 'learner',
                'login' => '13' . ($i + 2) . random_int(10000000, 99999999),
                'password_hash' => 'x',
    ...
    private function enable(): void
    {
        ...
        $staff = (int) Db::name('accounts')->insertGetId([
            'kind' => 'staff',
            'login' => 'uniq-' . bin2hex(random_bytes(4)),
            'password_hash' => 'x',
            'must_change_password' => 0,
            'status' => 'active',
            'created_at' => $now,
    ...
    private function succeededOrder(int $learnerId, float $paid): int
    {
        ...
        $categoryId = (int) Db::name('categories')->insertGetId([
            'parent_id' => 0,
            'name' => 'uniq-cat-' . bin2hex(random_bytes(2)),
            'path' => '/',
            'depth' => 1,
            'sort' => 0,
            'status' => 'enabled',
