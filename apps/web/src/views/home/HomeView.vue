      ...
      <section v-if="session.loggedIn" class="learning-action" data-testid="learning-action">
        ...
        <div v-else-if="action" class="learning-action__body" :data-action-state="actionState">
          ...
          <router-link
            ...
            class="btn btn-primary learning-action__link"
          >
      ...
      >
        ...
        <div class="home__map-grid">
          ...
          >
            ...
            <div>
              <h3>{{ map.title }}</h3>
              <p v-if="map.summary">{{ map.summary }}</p>
            </div>
...
const selectedPath = computed(() =>
  selectedId.value === null ? [] : findPath(categories.value, selectedId.value),
);
  ...
  try {
    const items =
      selectedId.value === null
        ? recentCourses.value
        : (await fetchCategoryCourses(selectedId.value, 1, 100)).list.items;
    if (request === courseRequest) courses.value = items;
  } catch {
    if (request === courseRequest) {
...
.learning-action .learning-action__eyebrow {
  font-size: 12px;
  color: var(--seal);
}
...
.category-detail :deep(.el-select) {
  width: 220px;
}
.course-grid {
  display: grid;
...
.home__map-card:nth-child(even) .map-icon {
  color: var(--gold);
}
.map-arrow {
  flex-shrink: 0;
